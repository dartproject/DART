...
<div align="left">
	...
	<font size="2">Here is a &quot;weighted list&quot; of the common relative
		...
		The more students is this group that show any standard as having no toggled status</font>
	<font size="2">, Emerging status, or Developing status, the larger the name
	...
	<div align="left">
		<ul>
			<li type="circle"><font size="2">Roll over each content area code to read it!<br/></font>
			<li type="circle"><font size="2">Click on the Level Number in any Content Area to view the <i>students in this group at that level</i>.<br/></font>
			<li type="circle"><font size="2" color="black">Click on the name of the student to see<i> his or her standards progress in all content areas</i>. </font><font size="2"><br/></font>
			<li type="circle"><font size="2">Click on any Content Area to <i>re-sort the view by Content Area &amp; Level Number</i>.<br/></font>
		</ul>
	</div>
</div>
...
{foreach $tabs as $id=>$t}
	...
	{foreach $t.subtabs as $s}
		{if $s.kind == 'iframe'}
			<iframe width='{$s.width}' height='{$s.height}' src='{$s.value}'></iframe><br/>
		{else}
			{$s.value}<br/>
		{/if}
	{/foreach}
	...
	<font size="4">
		<table width="100%" border= 1 class=big>
			...
			<tr bgcolor="{cycle values="$color1,$color2"}">
				...
				{foreach $subject as $s}
					{if $s.core == 'x'}
						<td><a href="index.php?cmd=toggle&p1={$listID}&p2={$student.{$l.studentid}.{$s.id}}">{$student.{$l.studentid}.{$s.id}}</a></td>
					{/if}
				{/foreach}
				<td>
				{foreach $subject as $s}
					{if $s.core != 'x' && $student.{$l.studentid}.{$s.id} != ''}
						<a href="index.php?cmd=toggle&p1={$listID}&p2={$s.id}&p3={$student.{$l.studentid}.{$s.id}}">{$s.id}.{$student.{$l.studentid}.{$s.id}}</a>
					{/if}
				{/foreach}
				</td>
			</tr>
